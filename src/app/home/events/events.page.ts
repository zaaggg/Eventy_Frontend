...
import {
  ...
  LoadingController, IonTitle } from '@ionic/angular/standalone';
...
export class EventsPage implements OnInit {
  ...
  getEvent(id: string) {
    this.event = this.eventS.requestedEvents.find((event) => event._id == id)!;
  }
  ...
  async cancelRequest() {
    ...
    this.participationS.cancelParticipationRequest(this.event._id).subscribe({
      next: async (response) => {
        await loading.dismiss();

        const alert = await this.alertController.create({
          header: 'Success',
          message: response.message || 'Request canceled successfully!',
          buttons: ['OK'],
        });
